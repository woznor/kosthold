<template>
      ...
      <v-card>
        ...
        <v-card-text>
          <v-form>
            ...
            />
            <v-checkbox
              v-model="meal.heatable"
              label="Kan retten varmes?"
            />
            <v-checkbox
              v-model="meal.must_be_heated"
              label="Må retten varmes?"
            />
        ...
        <v-card-actions>
          ...
          <v-btn color="error" @click="store.addMealDialog = false">
            Lukk
          </v-btn>
        </v-card-actions>
      </v-card>
  ...
  function initMeal() {
    meal.value = {
      ...
      protein_addons: [
        { text: 'Skyr', type: 'boks', grams: 160, number: 1 }
      ],
      nutrients: {
        calories: 400,
        protein: 41,
  ...
  function onSave() {
    ...
    store.saveMeal(meal.value)
      .then(() => {
        // If successful, close dialog or show a success message
        store.addMealDialog = false
      })
      .catch(err => {
        console.error(err)
